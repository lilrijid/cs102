...
config = {
    'VK_ACCESS_TOKEN': 'c2f5204a02199ae33906bbea10e06065fa0fc69b809ecdc7485326b50a485096291dd94f7773fa9d8a131',
    'PLOTLY_USERNAME': 'Имя пользователя Plot.ly',
    'PLOTLY_API_KEY': 'Ключ доступа Plot.ly',
    "VK_ID": 394481525,
    'DOMAIN': 'https://api.vk.com/method',
    'PLOTLY_USERNAME': 'rijid',
    'PLOTLY_API_KEY': 'pP6RpATeDpHzAGWzpZOS',    
}
...
def get_friends(user_id = config['VK_ID'], fields=''):
...
def age_predict(user_id = config['VK_ID']):
    ...
    assert isinstance(user_id, int), "user_id must be positive integer"
    assert user_id > 0, "user_id must be positive integer"
    information = get_friends(user_id, 'bdate')
    all_dates = [information['response']['items'][i]['bdate'] for i in range(information['response']['count']) if information['response']['items'][i].get('bdate')]
    dates = [all_dates[i] for i in range(len(all_dates))
             if len(all_dates[i]) >= 8]
    ages = [2017 - int(i[-4:]) for i in dates]
    result = int(sum(ages) / len(ages))
    return result


    
def messages_get_history(user_id = config['VK_ID'] , offset=0, count=20):
...
def count_dates_from_messages(messages):
    ...
    time = [date for date in dates_stat]
    stat = [dates_stat[date] for date in dates_stat]
    return time, stat
...
def get_network(user_id=config['VK_ID'], as_edgelist=True):
...
def plot_graph(user_id=config['VK_ID']):
